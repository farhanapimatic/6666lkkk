...
try
{
    client.Auth.Authorize(scope=new List<OAuthScopeEnum>(){OAuthScopeEnum.FDG, OAuthScopeEnum.DFG});
}
...
Scopes enable your application to only request access to the resources it needs while enabling users to control the amount of access they grant to your application. Available scopes are defined in the `BibcodeQuery.PCL.Models.OAuthScopeEnum` enumeration.
        ...
        static void Main(string[] args)
        {
           ...
           string oAuthClientSecret = "oAuthClientSecret"; // OAuth 2 Client Secret
            ...
            else
            {
                try
                {
                    storedToken = client.Auth.Authorize(new List<OAuthScopeEnum>(){OAuthScopeEnum.FDG, OAuthScopeEnum.DFG});
